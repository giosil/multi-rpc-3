...
class WMap implements Map
{
  ...
  public WMap(String sJSONObject)
  {
    if(sJSONObject == null || sJSONObject.length() < 2 || !sJSONObject.startsWith("{")) {
      map = new HashMap();
    }
    else {
      Object object = JSON.parse(sJSONObject);
  ...
  public String toQueryString() {
    ...
    while(iterator.hasNext()) {
      ...
      if(oVal instanceof Map) {
        sVal = JSON.stringify(oVal);
      }
      else {
        sVal = WUtil.toString(oVal, "null");
      }
